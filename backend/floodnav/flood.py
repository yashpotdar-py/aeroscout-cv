...
class FloodSegmenter:
    ...
    def _preprocess(self, image: np.ndarray) -> torch.Tensor:
        ...
        if image.shape[0] < 64 or image.shape[1] < 64:
            raise ValueError(f"Image too small: {image.shape[:2]}. Minimum size is 64x64.")
        
        # Convert to PIL for resizing
        pil_image = Image.fromarray(image.astype(np.uint8))
        pil_image = pil_image.resize((self.img_size, self.img_size), Image.Resampling.BILINEAR)
    ...
    def predict_with_resize(
        ...
        if output_size is not None:
            ...
            conf_pil = conf_pil.resize((output_size[1], output_size[0]), PILImage.Resampling.BILINEAR)
...
class ImprovedFloodSegmenter:
    ...
    def predict_with_resize(
        ...
        if output_size is not None:
            ...
            conf_pil = conf_pil.resize((output_size[1], output_size[0]), Image.Resampling.BILINEAR)
            confidence = np.array(conf_pil, dtype=np.float32) / 255.0
